...
##反省<br>
　構造型プログラミングの思考に則ったプログラムは作成できていると思うが、オブジェクト指向なプログラムにはなっていない。<br>
　弾と敵を別々の配列として保持しているが、データ構造を配列ではなくリストにし、t_cellを拡張し、衝突の判定をセル内で行うようにすればより計算量が少なく、分かりやすいプログラムになっただろう。move_objectに衝突の判定までやらせるのは仕事のさせすぎだった。<br>
　構造化がまだ足りていない。screen_initが生のデータを持っているのは良くない。もっと分割すべきだったし、screen_initは直接、値を操作するのではなく下位クラスのinitを呼ぶべきだった<br>
